...
export default function Admin() {
  ...
  const canSubmit = Boolean(form.title && form.technique && form.year && form.imageFile);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-10 mb-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xl font-semibold">Ordre des œuvres (glisser‑déposer)</h3>
            <button onClick={handleSaveOrder} className="bg-blue-600 hover:bg-blue-700 text-white rounded px-4 py-2 font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400">Enregistrer l'ordre</button>
          </div>
                  ...
                  <div ref={provided.innerRef} {...provided.droppableProps} className="flex flex-col gap-2">
                    {artworksOrder.map((a, index) => (
                      <Draggable draggableId={String(a.id)} index={index} key={a.id}>
                        {(prov) => (
                          <div ref={prov.innerRef} {...prov.draggableProps} {...prov.dragHandleProps} className="flex items-center gap-3 bg-black/40 border border-white/10 rounded p-2">
                            <span className="w-8 text-center text-sm opacity-70">{index + 1}</span>
                            <img src={a.imageUrl} alt={a.title} className="w-14 h-14 object-cover rounded border border-white/10" />
